...
def video2frame(infile: Path, hz: float, outfile: Path):
    ...
    period = round(fps / hz)
    print(f"Starting operation with {fps} fps and {hz} Hz, {infile}, skipping {period-1} frames")

    while True:
        counter += 1

        camera.set(cv2.CAP_PROP_POS_FRAMES, max((counter * period) - 1, 1))
