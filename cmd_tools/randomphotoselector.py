...
parser = argparse.ArgumentParser(
    description="Selects random photos from given folder"
)
...
parser.add_argument(
    ...
    type=float,
    default=1,
    help='number of photos you want to select'
)
...
def photoselector(infile: Path, count: int, outfolder: str, copyfile: Path):
    ...
    for f in randomfiles:
        ...
        shutil.move(
            f, copyfile
        )  #TODO bu kısım sıkıntılı. Daha iyi bir hale getirmemiz lazım. Asıl dosyaya dokunulmamalı. Onun yerine kopyalanıp taşınmalı.


if __name__ == '__main__':
    photoselector(
        Path(opt.infolder), int(opt.count), str(opt.outfolder), str(opt.copy)
    )
