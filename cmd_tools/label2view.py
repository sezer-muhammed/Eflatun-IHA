...
label_loader_mapping = {"yolo": ct.TEKNOLABEL_TYPE_YOLO, "pascal": ct.TEKNOLABEL_TYPE_PASCAL}
...
for image in images:
    ...
    label_path = os.path.join(in_fold, image.replace(".jpg", label_type_mapping[opt.label]).replace(".png", label_type_mapping[opt.label]))
    label = label_loader(label_path, config, img_path, label_loader_mapping[opt.label])
    ...
    for l in label_data:
        color = (hash_255(l[-1] + "B"), hash_255(l[-1] + "G"), hash_255(l[-1] + "R"))
        x1, y1, x2, y2 = l[0], l[1], l[2], l[3]
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        cv2.putText(frame, l[4], (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 2.5, color, 3)
    ...
    if opt.save:
        Path(os.path.join(in_fold, "labeled_output")).mkdir(parents=True, exist_ok=True)
